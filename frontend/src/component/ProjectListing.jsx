...
const handleFollow = async (projectId) => {
  ...
  try {
    await axios.post(
      `http://localhost:3000/api/projects/${projectId}/follow`,
      {},
      {
        headers: {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        ...
        const projectsData = response.data.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)); // Sort projects by createdAt in descending order
  ...
  const handleNotificationAction = async (notificationId, action, projectId) => {
    console.log(`Notification ID: ${notificationId}, Action: ${action}, Project ID: ${projectId}`);
    const token = localStorage.getItem('token');
    if (!token) {
      console.error('No token found in local storage');
      return;
    ...
    try {
      await axios.post(`http://localhost:3000/api/notifications/${notificationId}/handle`, { action }, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleLike = async (projectId) => {
    ...
    try {
      await axios.post(`http://localhost:3000/api/projects/${projectId}/like`, {}, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleComment = async (projectId) => {
    ...
    try {
      const response = await axios.post(`http://localhost:3000/api/projects/${projectId}/comment`, { comment: commentText[projectId] }, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 py-6 flex flex-col justify-center sm:py-12">
      <div className="container mx-auto px-4">
        <h1 className="text-4xl font-bold text-gray-900 dark:text-gray-100 mb-8 text-center">Project Listings</h1>
        <div className="grid gap-6 lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1">
        {loading ? (
            Array.from({ length: 6 }).map((_, index) => (
              <div key={index} className="relative bg-white dark:bg-gray-800 shadow-lg rounded-lg overflow-hidden transform transition duration-300 hover:scale-105 hover:shadow-2xl">
                <Skeleton height={192} />
                <div className="p-4">
                  <Skeleton height={20} width="80%" style={{ marginBottom: '8px' }} />
                  <Skeleton height={16} width="50%" />
                  <Skeleton height={16} width="70%" style={{ marginTop: '8px' }} />
                </div>
              </div>
            ))
          ) : (
              ...
              >
                ...
                <div className="flex items-center mb-2">
                    <img
                      ...
                      src={project.user.profileImage || `https://ui-avatars.com/api/?name=${project.user.username}&background=random&size=512`}
                      alt={project.user.username}
                    />
                    <p className="text-gray-700 dark:text-gray-300 font-bold">{project.user.username}</p>
                  </div>
                  <img className="h-48 w-full object-cover" src={`https://ui-avatars.com/api/?name=${project.title}&background=random&size=512`} alt={project.title} />
                  <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black to-transparent p-4">
                    <h2 className="text-lg font-medium text-white">{project.title}</h2>
                    <p className="text-gray-300 mt-1 truncate">{project.description}</p>
                  </div>
                  {likedProject === project._id && (
                    <div className="absolute inset-0 flex items-center justify-center">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" className="w-24 h-24 text-red-500 animate-ping" viewBox="0 0 24 24">
                ...
                <div className="p-4">
                  <div className="flex items-center justify-between mb-2">
                    <button onClick={() => handleLike(project._id)} className="flex items-center text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-200">
                      {userLikes[project._id] ? (
                        <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" className="w-6 h-6 mr-1" viewBox="0 0 24 24">
                          <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/>
                        </svg>
                      ) : (
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" stroke="currentColor" className="w-6 h-6 mr-1" viewBox="0 0 24 24">
                          <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/>
                        </svg>
                      )}
                      Like
                    </button>
                    <span className="text-gray-600 dark:text-gray-400">{project.likes && project.likes.length > 0 ? project.likes.length : 0}</span>
                  </div>
                  <div className="flex items-center justify-between mb-2">
                    <button onClick={() => handleFollow(project._id)} className="flex items-center text-green-600 dark:text-green-400 hover:text-green-800 dark:hover:text-green-200">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" className="w-6 h-6 mr-1" viewBox="0 0 24 24">
                    ...
                    <span className="text-gray-600 dark:text-gray-400">{project.followers && project.followers.length > 0 ? project.followers.length : 0}</span>
                  </div>
                  <div className="flex items-center mb-2">
                    <button onClick={() => toggleComments(project._id)} className="flex items-center text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-200">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" className="w-6 h-6 mr-1" viewBox="0 0 24 24">
                  ...
                  {showComments[project._id] && (
                    <div className="mb-4">
                      ...
                      alt={comment.avatar}
                    />
                          <p className="text-gray-700 dark:text-gray-300 text-sm font-bold mr-2">{comment.username}</p>
                  ...
                  <Link to={`/projects/${project._id}`} className="text-indigo-600 dark:text-indigo-400 hover:underline">View Details</Link>
                </div>
              </div>
            ))
          )}
  ...
  <ul className="space-y-4">
    {notifications.map((notification) => (
      <li key={notification._id} className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-lg">
        <p className="text-gray-700 dark:text-gray-300">{notification.message}</p>
        {notification.type === 'followRequest' && (
          <div className="mt-2 flex space-x-4">
            <button
              onClick={() => handleNotificationAction(notification._id, 'accept', notification.projectId)}
              className="bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-700"
            >
              Accept
            </button>
            <button
              onClick={() => handleNotificationAction(notification._id, 'reject', notification.projectId)}
